...
const create = catchError(async(req, res) => {
    ...
    await sendEmail({
        to: email,
        subject: 'Verificate email for user email',
        html: `
...
const update = catchError(async(req, res) => {
    const { id } = req.params;
    const { firtsName, lastNama, country, image } = req.params;
    const result = await User.update(
        { firtsName, lastNama, country, image },
...
const login = catchError(async(req, res) => {
    ...
    if (!user) return res.status(401).json({ message: {email} });
